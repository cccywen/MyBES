...
# 每个stage都加空间关系模块 逐步(没有逐步就一次而已)上采,上采过程加上了1 3层的原始特征
...
class BESNet(nn.Module):
    def __init__(self, nclass, backbone='resnet18', aux=True, norm_layer=nn.BatchNorm2d, pretrained=True):
        ...
        self.attention0 = PAM_Module(64)
        self.attention1 = PAM_Module(64)
        self.attention2 = PAM_Module(128)
        self.attention3 = PAM_Module(256)
        self.attention4 = PAM_Module(512)
        ...
        self.up1 = UP_Conv(128, 128)  # ch_in, ch_out
        self.up2 = UP_Conv(896, 128)  # ch_in, ch_out
        self.Enhance = BES_Module(512, 128)  # f5_in, mul_ch
        self.head = FCNHead(384, nclass, norm_layer)  # in_channels, out_channels, norm_layer=nn.BatchNorm2d
        if self.aux:
    ...
    def forward(self, x):
        ...
        a3 = c3 + self.attention3(c3)
        a2 = self.attention2(c2)
        a1 = c1 + self.attention1(c1)
        a0 = self.attention0(c0)


        x = self.Enhance(c4, b, f)
        # x = x + self.attention(c1)
        x = torch.cat((x, a0), dim=1)
        x_high = torch.cat((x, a1), dim=1)

        x_low = torch.cat((a2, a3), dim=1)
        x = torch.cat((x_low, a4), dim=1)
        up_x = self.up2(x)
        x = torch.cat((up_x, x_high), dim=1)
        x = self.head(x)
        x = Upsample(x, imsize)
