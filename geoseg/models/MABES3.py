...
# 返回一个tuple 含x和与boundary GT做loss的bl 先修改了只输出一个 需要的话要改回
...
class BESNet(nn.Module):
    def __init__(self, nclass, backbone='resnet18', aux=True, norm_layer=nn.BatchNorm2d, pretrained=True):
        ...
        self.head = FCNHead(448, nclass, norm_layer)  # in_channels, out_channels, norm_layer=nn.BatchNorm2d
        if self.aux:
    ...
    def forward(self, x):
        ...
        x = torch.cat((x, self.attention1(c1)), dim=1)
