...
class BESNet(nn.Module):
    def __init__(self, nclass, backbone='resnet18', aux=True, norm_layer=nn.BatchNorm2d, pretrained=True):
        ...
        self.attention = PAM_Module(filters[1])
        ...
        self.visi_output = nn.Conv2d(256, 3, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        x = torch.cat((x, self.attention(c2)), dim=1)
        x_tmp = x.clone()   # 输出128 channel
