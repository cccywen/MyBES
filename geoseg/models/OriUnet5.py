...
# 下采 加权上采 每次上采后加两次卷积 加ASPP 有边缘信息
...
class BE_Module(nn.Module):
    def __init__(self, in_ch1, in_ch2, in_ch5, mid_ch, out_ch, n_class):
        ...
        self.max = nn.MaxPool2d(kernel_size=2, stride=2, padding=0, dilation=1)
    ...
    def forward(self, l1, l2, l4):
        l1_b = self.convb_1(l1)
        l1_b = self.max(l1_b)
        l2_b = self.convb_2(l2)
        l4_b = self.convb_5(l4)
        l4_b = F.interpolate(l4_b, l2_b.size()[2:], mode='bilinear', align_corners=True)

        b = torch.cat((l1_b, l2_b, l4_b), dim=1)  # 输出的boundary feature Fb
